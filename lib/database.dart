...
class DatabaseHelper {
  ...
  Future<int> insertSavings(double amount, String category,
      String iconPath) async {
    Database db = await database;
    return await db.insert('savings',
        {'amount': amount, 'category': category, 'iconPath': iconPath});
  }
  ...
  Future<bool> checkCategoryExists(String category) async {
    ...
    var result = await db.query(
        'savings', where: 'category = ?', whereArgs: [category]);
  ...
  Future<int> updateCategoryNameSavings(String oldCategoryName,
      String newCategoryName) async {
    Database db = await database;
    return await db.update(
        'savings', {'category': newCategoryName}, where: 'category = ?',
  ...
  Future<int> insertLoans(double amount, String category,
      String iconPath) async {
    Database db = await database;
  ...
  Future<String> getCategoryIconsLoans(String category) async {
    final db = await database;
    var result = await db.query(
        'loans', where: 'category = ?', whereArgs: [category]);
    if (result.isNotEmpty) {
      return result.first['iconPath'] as String;
    } else {
      return '';
  ...
  Future<int> updateCategoryNameLoans(String oldCategoryName,
      String newCategoryName) async {
    Database db = await database;
    return await db.update(
        'loans', {'category': newCategoryName}, where: 'category = ?',
        whereArgs: [oldCategoryName]);
  }
  ...
  Future<int> insertAccounts(double amount, String category,
      String iconPath) async {
    Database db = await database;
  ...
  Future<bool> checkAccountsCategoryExists(String category) async {
    ...
    var result = await db.query(
        'accounts', where: 'category = ?', whereArgs: [category]);
  ...
  Future<int> updateAccountsCategoryName(String oldCategoryName,
      String newCategoryName) async {
    Database db = await database;
    return await db.update(
        'accounts', {'category': newCategoryName}, where: 'category = ?',
        whereArgs: [oldCategoryName]);
  }
  ...
  Future<Map<String, dynamic>> getAccountById(int id) async {
    ...
    List<Map<String, dynamic>> accounts = await db.query(
        'accounts', where: 'id = ?', whereArgs: [id], limit: 1);
  ...
  Future<int> insertPlans(double plannedAmount, String category) async {
    ...
    // Получение данных из таблиц savings и accounts
